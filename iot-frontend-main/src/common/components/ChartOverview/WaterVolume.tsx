...
const WaterVolumeChart = () => {
  ...
  const {
    ...
    queryFn: () => getWaterVolumeData({ area: activeRegion, interval: chartPeriod }),
  ...
  // const { options } = useChartOptions({ chartPeriod, seriesData: waterVolumeData?.data.series });
  const options = initWaterChartOptions({ chartPeriod, seriesData: waterVolumeData?.data });

  return (
    <Box as="section" className="rgc-water-use-chart" w="full">
      <Flex justify="space-between" align="center" w="full" mb="12px">
        <Heading size="sm">Tingkat Air per {isAdmin ? "Kota/Kabupaten" : "Desa"} (m)</Heading>
        <Box minW={{ base: "5rem", md: "15rem", lg: "25rem" }}>
          <Select defaultValue={DEFAULT_PERIOD} onChange={e => setChartPeriod(e.target.value as IChartPeriod)}>
      ...
      {waterVolumeData?.data.series && !isFetching ? (
        <Chart options={options} series={waterVolumeData?.data.series} type="area" height={350} />
      ) : !isFetching ? (
